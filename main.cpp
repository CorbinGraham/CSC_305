...
void Triangle::loadShaders()
{
	...
	if (auto result = glx::linkShaders(mProgramHandle);
		result)
	{
		fmt::print("{}\n", *result);
		std::abort();
...
void Triangle::loadDataToGPU(
   std::array<float, 18> const& vertices)
{
...
{
	// If shader needs to be reloaded. reload it (for each of the 2 shaders)
}

void Triangle::render()
{
	//Call to reload shaders
...
void Triangle::freeGPUData()
{
	...
	//gl_delete all the stuff 5 things.
}
